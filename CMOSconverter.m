...
if strcmp(oldfilename(end-2:end),'gsh')
    ...
    if size(camType,2) == 0
        ...
        indEnd=strfind(fstr(ind:end),13);
        indEnd=indEnd(1)+ind;
        acqFreq = str2double(fstr(ind+21:indEnd)); % in msec
        frequency = 1000/acqFreq;
        dual = 0;
    end
    % Grab data %
...
if strcmp(oldfilename(end-2:end),'tif')
    f = waitbar(0,'Read image size...');
    file = dirname+"/"+oldfilename;
    %filename = '/home/langem/Documents/millimeterEnc/confocal/opticalMapping/c21-043/confocal/CL400ms-2.tif';
    tfInfo =  imfinfo(file);
   % tf = Tiff(file,'r');
    timeSteps= size(tfInfo,1);
    cmosData = zeros([tfInfo(1).Width tfInfo(1).Height timeSteps]);
    
    waitbar(.01,f,'Loading image');
    for i = 1:timeSteps
        waitbar(i/timeSteps,f,'Loading image');
        cmosData(:,:,i)=imread(file,i);
    end
    frequency=500;
% setDirectory(tf,dirNum);
% ti = read(tf);
% timeSteps= 499;
% series= zeros(timeSteps,size(ti,1),size(ti,2));
% for i = 1:timeSteps 
%     series(i,:,:) = read(tf);
%     nextDirectory (tf) 
% end
