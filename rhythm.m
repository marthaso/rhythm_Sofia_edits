...
% When mouse button is clicked and held find associated marker
    function [i_temp,j_temp] = button_down_function(obj,a,h)
        ...
        if handles.activeCamData.isloaded && handles.drawBrush
            ...
            if handles.drawBrush
                ...
                if i_temp<=size(handles.activeCamData.cmosData,1) && i_temp>1 && ...
                    ...
                    button=get(gcf,'selectiontype'); % The left mouse button is normal, where the right is alt. 
                    %ctrlIsPressed = ismember('control',modifiers);
                    ctrlIsPressed = strcmp(button,'alt');
...
%% Select directory for optical files
    function selectdir_callback(~,~)
        ...
        if dir_name ~= 0
            ...
            search_nameRaw = [dir_name,'/*.raw'];
            search_nameTif = [dir_name,'/*.tif'];
            files = struct2cell(dir(search_name));
            filesNew = struct2cell(dir(search_nameNew));
            filesMat = struct2cell(dir(search_nameMat));
            filesRaw = struct2cell(dir(search_nameRaw));
            filesTif = struct2cell(dir(search_nameTif));
            handles.file_list = [files(1,:)'; filesNew(1,:)';filesMat(1,:)';filesRaw(1,:)';filesTif(1,:)'];
    ...
    function dispwave_button_callback(~,~)
        ...
        if i_temp>size(handles.activeCamData.cmosData,1) ||...
            ...
            elseif handles.bounds(handles.activeScreenNo) == 2
             ...
             %%   a = 1;
             %%   signal_current = filter(b,a,signal_current);
               %signal_current = diff([signal_current;signal_current(end)]);
                plot(handles.time(1:handles.activeCamData.maxFrame),...
                    signal_current,...
                    handles.markerColors(wave_window),'LineWidth',2,...
                    'Parent',handles.signalScreens(wave_window));
                if size(handles.activeCamData.savePicks,1)>j 
                    %f=figure();
                    %ax= gca();
                    %plot(handles.time(1:handles.activeCamData.maxFrame),...
                    %    signal_current,...
                    ...
                    pickId=handles.activeCamData.savePicks(j,i);
                    %axis (handles.signalScreens(wave_window));
                    hold all;
                    tt= handles.time(handles.activeCamData.savePicks(j,i));
                    td= signal_current(handles.activeCamData.savePicks(j,i));
                    plot(handles.time(1:handles.activeCamData.maxFrame),...
                        signal_current,...
                        handles.markerColors(wave_window),...                        
                        handles.time(handles.activeCamData.savePicks(j,i)),...
                        signal_current(handles.activeCamData.savePicks(j,i)),...
                        'b*','LineWidth',2,...
                        'Parent',handles.signalScreens(wave_window));
                    hold off;
                end
                %handles.activeCamData.savePicks
