...
% Function Summary: This code will create a quiver map and a color map of
% conduction velocity. (multiple functions called)

% Inputs: Don't need most of the inputs if we are doing avg act map. Should
% edit later.

% Outputs: A quiver plot, a histogram of velocity magnitudes, a color map
...
% make two matrices. both are the size of your rectangle (ROI) in xx each
% COLUMN is an x coord in yy each ROW is a y coord
[xx, yy] = meshgrid(rect(1):rect(1)+rect(3), rect(2):rect(2)+rect(4));
% put it all in a vector. so you have x1 repeated however many y pixels you
% have, x2 repeated however many y pixels you have, etc.
xx = reshape(xx,[],1);
yy = reshape(yy,[],1);
...
M = size(xyt,1);
% User input for the pixel resolution.
prompt = "Resolution? ";
xres = input(prompt);
        ...
        %     %     second(spp,timm) = nanmean(Velval);
            ...
            for ds_fac = 6 % Define downsampling factor. 6 seems to work well.
                ...
                for auto = 2 % Autoscale factor. 2 seems to work well.
                    ...
                    Velval = reshape(Velval,size(croppedAmap,1),size(croppedAmap,2));
                    % Make a matrix of zeros the size of your FOV
                    maskvel = zeros(size(bg));
                    % Place your velocity matrix in the right position
                    % (ROI)
                    maskvel(rect(2):rect(2)+rect(4),rect(1):rect(1)+rect(3)) = Velval;
                    % Turn any nan values into 0
...
% Function Summary: Take all the velocity values and downsample them for
% the quiver plot.
% Inputs: An activation map of your ROI, a vector with all the x coords, a
% vector with all the y coords, your x velocities, your y velocities, and
% how much you want to downsample.
% Outputs: Four line vectors. X has the x coords of the pixels you will
...
% Inputs: M (how many pixels you have), xyt (matrix containing x coord, y
% coord, and t of your pixels), space_window_width (which pixels will be
