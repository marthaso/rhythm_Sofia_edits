...
%% Automatic running of APD code with figure generation


function [APD50_vec, APD80_vec] = auto_CaT_STUDY(start, endp, study, file_number,filename_pdf, AP_num,firstAP,lastAP)
    ...
    for b1 = 1: length(bin)
        for f1 = 1: length(freq)
            ...
            if firstAP == 1
                ...
                start = [locs(firstAP-1:lastAP-1)-10'];                
            end
            ...
            if ~isfile(sprintf('%s_Ca_bin%g_%gHz_SNR%g.mat',study,b,f,s))
                SNR_mask = SNR_mask_calculation(data,start, endp, Fs,s);
            ...
            for i = 1 : length(start)
                ...
                filename_filtered2 = sprintf('_CaT50_bin%g_%gHz_SNR%g_AP%g_matrix_90_hdi.mat', b, f, s, AP_num);
                directory = [fullfile('C:', 'Users', 'Sofia', 'Desktop', 'PAH', 'CaTMaps', study, num2str(file_number))];
                filename = append(directory,filename_filtered2);
                save(filename, 'filtered2');

                filename_1 = sprintf('_CaT50_bin%g_%gHz_SNR%g_AP%g_all.mat', b, f, s, AP_num);
                filename = append(directory,filename_1);
                save(filename, 'apdMap1');
                % 
                filename_1 = sprintf('_CaT50_bin%g_%gHz_SNR%g_AP%g_AP_start_point.mat', b, f, s,AP_num);
                filename = append(directory,filename_1);
                save(filename, 'AP_start_points');

                filename_1 = sprintf('_CaT50_bin%g_%gHz__AP%g_AP_end_points.mat', b, f,AP_num);
                filename = append(directory,filename_1);
                save(filename, 'AP_end_points');

                filename_1 = sprintf('_CaT50_bin%g_%gHz_SNR%g__AP%g_AP_baselines.mat', b, f, s,AP_num);
                filename = append(directory,filename_1);
                save(filename, 'AP_baselines');

                filename_1 = sprintf('_CaT50_bin%g_%gHz_SNR%g_AP%g_AP_storage.mat', b, f, s,AP_num);
                filename = append(directory,filename_1);
    ...
    for b1 = 1: length(bin)
        for f1 = 1: length(freq)
            ...
            for i = 1: length(start)
                ...
                [apdMap1, filtered2, AP_start_points ,AP_baselines, AP_end_points, AP_storage, APD_average] = CaTMap_STUDY(data, start(i), endp(i), 20,...
                    210, 80, area_coords, Fs, cmap, movie_scrn, handles, SNR_mask,f,b,filename_pdf,s,study);
%180
                APD80_vec = [APD80_vec, APD_average];
                AP_num = i;

                filename_filtered2 = sprintf('_CaT80_bin%g_%gHz_SNR%g_AP%g_matrix_90_hdi.mat', b, f, s, AP_num);
                directory = [fullfile('C:', 'Users', 'Sofia', 'Desktop', 'PAH', 'CaTMaps', study, num2str(file_number))];
                filename = append(directory,filename_filtered2);
                save(filename, 'filtered2');

                filename_1 = sprintf('_CaT80_bin%g_%gHz_SNR%g_AP%g_all.mat', b, f, s, AP_num);
                filename = append(directory,filename_1);
                save(filename, 'apdMap1');

                filename_1 = sprintf('_CaT80_bin%g_%gHz_SNR%g_AP%g_AP_start_point.mat', b, f, s,AP_num);
                filename = append(directory,filename_1);
                save(filename, 'AP_start_points');

                filename_1 = sprintf('_CaT80_bin%g_%gHz__AP%g_AP_end_points.mat', b, f,AP_num);
                filename = append(directory,filename_1);
                save(filename, 'AP_end_points');

                filename_1 = sprintf('_CaT80_bin%g_%gHz_SNR%g__AP%g_AP_baselines.mat', b, f, s,AP_num);
                filename = append(directory,filename_1);
                save(filename, 'AP_baselines');

                filename_1 = sprintf('_CaT80_bin%g_%gHz_SNR%g_AP%g_AP_storage.mat', b, f, s,AP_num);
                filename = append(directory,filename_1);
                save(filename, 'AP_storage');
            end
