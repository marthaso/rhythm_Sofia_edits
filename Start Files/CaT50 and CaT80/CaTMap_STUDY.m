...
for pix_x = i_min : i_max
    ...
    for pix_y = j_min : j_max
        ...
        if max(data(pix_x,pix_y,start:endp))==0
            ...
            range_start = 10;
            range_end = 10;
            pixel_calc = squeeze(pixel_calc(range_start:end-range_end));
            % I found these values to work in our data, can adjust as
            % needed.
            [pks, locs] =findpeaks(pixel_calc,'MinPeakDistance',50,'MinPeakProminence',0.3,'MaxPeakWidth',130);
...
plot_examples(filtered2,data(:,:,start:endp),AP_start_points,AP_baselines,AP_end_points,AP_storage,filename_pdf, start, endp,range_start,range_end)
plot_excluded_examples(study,filtered2,all_data(:,:,start:endp),filename_pdf,range_start,range_end)
...
close(gcf)
    ...
    function plot_examples(apdMap1,data,AP_start_points,AP_baselines,AP_end_points,AP_storage,filename_pdf,start, endp,range_start,range_end) 
        ...
        for pix = 1:length(x)
            subplot(2,5,pix);
            pixel_example = squeeze(data(y(pix),x(pix),range_start:end-range_end));
    ...
    function [start_of_AP] = start_of_AP_calc(AP)
        ...
        %[peak, location] =findpeaks(AP,'MinPeakDistance',50,'MinPeakProminence',0.3,'MaxPeakWidth',110);
        [peak, location] =findpeaks(AP,'MinPeakDistance',50,'MinPeakProminence',0.3,'MaxPeakWidth',130);
    ...
    function plot_single_example (data, y, x, AP_baselines, AP_start_points, AP_end_points, AP_storage )
        ...
        plot(end_point,0:0.1:1,'LineWidth',1)
        title(sprintf('CaT = %d, x = %d, y = %d', AP_storage(y,x),y,x),'FontName','Arial','FontSize',5)
    end

    function plot_excluded_examples(study,apdMap1,data,filename_pdf,range_start,range_end) 
