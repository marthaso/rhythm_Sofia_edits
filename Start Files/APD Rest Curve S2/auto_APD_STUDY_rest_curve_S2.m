...
function [APD50_vec, APD80_vec] = auto_APD_STUDY_rest_curve_S2(start, endp, study, file_number,filename_pdf, AP_num,firstAP,lastAP)
...
for snr = 1:length(SNR_value)
    for b1 = 1: length(bin)
        for f1 = 1: length(freq)
            ...
            figure
            [pks, locs]=findpeaks(pixel_calc,'MinPeakDistance',50,'MinPeakProminence',0.15,'MaxPeakWidth',110);
            ...
            for i = 1 : length(start)
                ...
                [apdMap1, filtered2, AP_start_points ,AP_baselines, AP_end_points, AP_storage, APD_average] = apdMap_STUDY_rest_curve_S2(data, start(i), endp(i),...
    ...
    for b1 = 1: length(bin)
        for f1 = 1: length(freq)
            ...
            for i = 1: length(start)
                ...
                [apdMap1, filtered2, AP_start_points ,AP_baselines, AP_end_points, AP_storage, APD_average] = apdMap_STUDY_rest_curve_S2(data, start(i), endp(i), 20,...
