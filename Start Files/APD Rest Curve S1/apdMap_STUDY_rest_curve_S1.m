function [apdMap1,filtered2,AP_start_points,AP_baselines,AP_end_points,AP_storage, APD_average] = apdMap_B4M4_rest_curve_S1(data,...
    start, endp,...
    minapd, maxapd,...
    percentAPD,...
    area_coords,...
...
for pix_x = i_min : i_max
    for pix_y = j_min : j_max
        ...
        if max(data(pix_x,pix_y,start:endp))==0
            ...
            pixel_calc = squeeze(pixel_calc(20:end-10));
            % I found these values to work in our data, can adjust as
            % needed.
            % [pks, locs] =findpeaks(pixel_calc,'MinPeakDistance',50,'MinPeakProminence',0.,'MaxPeakWidth',110);
...
% figure
% histogram(apdMap1(:), 'BinWidth', 5)
% plot_examples(apdMap1,data,AP_start_points,AP_baselines,AP_end_points,AP_storage) % plot some example pixels. can comment out if you don't want these extra figures.
% save('C:\Users\Sofia\Desktop\PAH\CaTMaps\Jan14\CaT50_matrix_all.mat','apdMap1') %change to your directory
...
% filtered_apdMap = apdMap1;
% filtered_apdMap(filtered_apdMap<prctile(apdMap1(:),5))=NaN;
% filtered_apdMap(filtered_apdMap>prctile(apdMap1(:),95))=NaN;
...
% plotapd(apdMap1,percentAPD,Hz,filename_pdf)
...
% Save the SNR mask
% save('C:\Users\Sofia\Desktop\PAH\APDMaps\Aug6\APD_bin3_SNR_mask.mat','SNR_mask') %change to your directory
...
% print(fs,filename_pdf,'-dpsc','-fillpage','-append')


% y=110;
% x=110;
% plot_single_example(data, y, x, AP_baselines, AP_start_points, AP_end_points, AP_storage )
...
close(gcf)
    ...
    function plotapd(apdMap1,percentAPD,Hz,filename_pdf)
        ...
        colorbar
        axis image
        title(sprintf('APD%g - %g Hz - bin %g',percentAPD, Hz,b),'FontName','Arial','FontSize',12) %change as needed

        print(gcf,filename_pdf,'-dpsc','-fillpage','-append')
    ...
    function plot_examples(apdMap1,data,AP_start_points,AP_baselines,AP_end_points,AP_storage,filename_pdf,start, endp)
        ...
        if length(sortIdx) > 50
            ...
            for pix = 1:length(x)
                ...
                title(sprintf('APD = %d, x = %d, y = %d', AP_storage(y(pix),x(pix)),y(pix),x(pix)),'FontName','Arial','FontSize',5)
    ...
    function [end_of_AP] = end_of_AP_calc(AP,AP_baseline,AP_level)
        ...
        [~,max_place] = findpeaks(AP,'MinPeakDistance',110,'MinPeakProminence',0.10,'MaxPeakWidth',110);
    ...
    function [start_of_AP] = start_of_AP_calc(AP)
        ...
        [peak, location] =findpeaks(AP,'MinPeakDistance',110,'MinPeakProminence',0.10,'MaxPeakWidth',110);
        if location>50
            [~,start_of_AP] = max(diff(AP(location-50:location)));
            start_of_AP = location-50+start_of_AP;
        else
    ...
    % load og mask
        mask = load(sprintf('%s_V.txt',study));
