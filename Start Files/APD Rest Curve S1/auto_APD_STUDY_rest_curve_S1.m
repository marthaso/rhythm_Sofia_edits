...
function [APD50_vec, APD80_vec] = auto_APD_B4M4_rest_curve_S1(start, endp, study, file_number,filename_pdf, AP_num,firstAP,lastAP)
...
for snr = 1:length(SNR_value)
    for b1 = 1: length(bin)
        for f1 = 1: length(freq)
            ...
            f = freq(f1);
            s = SNR_value(snr);
            if ~isfile(sprintf('%s_%g_V_bin%g_%gHz.mat',study,file_number,b,f))
                % create the file
                directory = fullfile('C:', 'Users', 'Sofia', 'Downloads', study);
                file_name = sprintf('%g_CAM1.raw',file_number);
                ...
                title = sprintf('%s_%g_V_bin%g_%gHz.mat',study,file_number,b,f);
                save(title,'data','-v7.3');

            end
            data = load(sprintf('%s_%g_V_bin%g_%gHz.mat',study,file_number,b,f));
            ...
            figure
            % findpeaks(pixel_calc,'MinPeakDistance',50,'MinPeakProminence',0.2,'MaxPeakWidth',110)
            findpeaks(pixel_calc,'MinPeakDistance',50,'MinPeakProminence',0.3,'MaxPeakWidth',110)
            % pixel_calc = data(77,153,:);
            % pixel_calc = normalize_data(pixel_calc);
            % pixel_calc = squeeze(pixel_calc);
            ...
            if ~isfile(sprintf('%s_V_bin%g_%gHz_SNR%g.mat',study,b,f,s))
                SNR_mask = SNR_mask_calculation(data,start(1), endp(1), Fs,s);
                mask_name = sprintf('%s_V_bin%g_%gHz_SNR%g.mat',study,b,f,s);
                save(mask_name,'SNR_mask')
            end
            SNR_mask = load(sprintf('%s_V_bin%g_%gHz_SNR%g.mat',study,b,f,s));
            ...
            for i = 1 : length(start)
                % disp('new apd')
                [apdMap1, filtered2, AP_start_points ,AP_baselines, AP_end_points, AP_storage, APD_average] = apdMap_B4M4_rest_curve_S1(data, start(i), endp(i),...
                    20, 150, 50, area_coords, Fs, cmap, movie_scrn, handles, SNR_mask,f,b,filename_pdf,s,study);
               
                APD50_vec = [APD50_vec, APD_average];
                AP_num = i;

                % filename_filtered2 = sprintf('APD50_bin%g_%gHz_SNR%g_AP%g_matrix_90_hdi.mat', b, f, s, AP_num);
                % directory = [fullfile('C:', 'Users', 'Sofia', 'Desktop', 'PAH', 'APDMaps', study) filesep];
    ...
    for b1 = 1: length(bin)
        for f1 = 1: length(freq)
            ...
            for i = 1: length(start)
                ...
                [apdMap1, filtered2, AP_start_points ,AP_baselines, AP_end_points, AP_storage, APD_average] = apdMap_B4M4_rest_curve_S1(data, start(i), endp(i), 20,...
                    180, 80, area_coords, Fs, cmap, movie_scrn, handles, SNR_mask,f,b,filename_pdf,s,study);
