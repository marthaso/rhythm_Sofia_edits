function SNR_mask = SNR_mask_calculation(data,start, endp, Fs, s)
%% SNR mask
...
% i_max = area_coords(2) + area_coords(4);
 %% BG time points
        ...
        % plot(1:length_of_data,squeeze(data(150,150,start:endp)));
        Fs = 1000;
        % start = start*Fs;
        % endp = endp*Fs;
        % data{1,1} = data{1,1}(:,:,start:endp);
        % data= cell2mat(data);
        % data = normalize_data(data);
        % plot(1:2000,squeeze(data(130,130,start+2500:start+1999+2500)));
        % 
        % Fs = 1000;
        % start = start*Fs;
        % endp = endp*Fs;
        ...
        plot(1:length_of_data,squeeze(data(150,150,start(1):endp(1)))); % aug26
        % plot(1:length_of_data,squeeze(data(174,172,:)));
        % plot(1:length_of_data,squeeze(data(172,70,start:endp))); % nov5
        ...
        % plot(1:length_of_data,squeeze(data(150,150,start:endp)))
        % plot(1:2000,squeeze(data(130,130,start+2500:start+1999+2500)));
        % plot(1:length_of_data,squeeze(data(130,130,start:endp)))
        plot(1:length_of_data,squeeze(data(150,150,start:endp))); % aug26
        % plot(1:length_of_data,squeeze(data(174,172,:)));
        % plot(1:length_of_data,squeeze(data(172,70,start:endp))); % nov5
